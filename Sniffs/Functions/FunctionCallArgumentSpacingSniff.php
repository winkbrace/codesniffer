 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class WinkBrace_Sniffs_Functions_FunctionCallArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
            ...
            if ($tokens[$nextSeparator]['code'] === T_COMMA)
            {
                ...
                else
                {
                    ...
                    if (strpos($tokens[($nextSeparator + 1)]['content'], $phpcsFile->eolChar) === false)
                    {
                        ...
                        if ($space > 1)
                        {
                            // check if the spaces are just there to line the comments out nicely
                            for ($i = $nextSeparator + 1; $i < count($tokens); $i++)
                            {
