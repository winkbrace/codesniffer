 ...
 * @link      http://pear.php.net/package/PHP_CodeSniffer
 */
class WinkBrace_Sniffs_Classes_ClassDeclarationSniff extends PEAR_Sniffs_Classes_ClassDeclarationSniff
{
    ...
    public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$openBrace]['line'] === $tokens[$closeBrace]['line'] && $tokens[$openBrace]['column'] === $tokens[$closeBrace]['column'] - 1)
        {
            $emptyBody = true;
        }
    ...
    public function processOpen(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        if (strlen($gap) !== 1 && strpos($gap, $phpcsFile->eolChar) === false)
        {
            $found = strlen($gap);
    ...
    public function processClose(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
    {
        ...
        // Check that the closing brace comes within 2 rows after the code body.
        $closeBrace  = $tokens[$stackPtr]['scope_closer'];
        $prevContent = $phpcsFile->findPrevious(T_WHITESPACE, ($closeBrace - 1), null, true);
        $prevLine    = $tokens[$closeBrace]['line'] - 1;
        if ($prevContent !== $tokens[$stackPtr]['scope_opener']
            && ! in_array($tokens[$prevContent]['line'], array($prevLine - 1, $prevLine))
            )
        {
            $error = 'The closing brace for the %s must go on the next two lines after the body';
